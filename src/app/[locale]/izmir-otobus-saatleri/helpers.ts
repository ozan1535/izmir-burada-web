...
export const getFormattedData = (data: IBusHour[]): FormattedDayData[] => {
  const result: FormattedDayData[] = [];
  let currentDayBusHours: IBusHour[] = [];
  ...
  data.forEach((item, idx) => {
    ...
    currentDayBusHours.push(item);
  });

  // Push the last group if there's remaining data
  if (currentDayBusHours.length > 0) {
    const dateObj = new Date(today);
    dateObj.setDate(today.getDate() + dayIndex);
...
export const fetchItems = async (
  hatNo: number,
  setData: Dispatch<SetStateAction<IOtobusSaatleri>>
) => {
  try {
